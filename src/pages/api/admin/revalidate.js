...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (scope === 'list') {
      ...
      } else if (listScope === 'gallery-ad' || listScope === 'vending' || listScope === 'announcement-popup' || listScope === 'popup-ad' || listScope === 'social-links') {
        paths = await collectGalleryAdRevalidatePaths()
      }
    ...
    if (scope === 'batch') {
      ...
      paths = await collectShellWithCustomPagePaths()
    } else if (scope === 'gallery-ad' || scope === 'vending' || scope === 'announcement-popup' || scope === 'popup-ad' || scope === 'social-links') {
      paths = await collectGalleryAdRevalidatePaths()
